...
public class ModRegistryDataGenerator extends DatapackBuiltinEntriesProvider {
    ...
    private ModRegistryDataGenerator(PackOutput output, CompletableFuture<HolderLookup.Provider> provider) {
        super(output, provider, BUILDER, Set.of("minecraft", Osmiumsmagic.MOD_ID));
    }
    ...
    public static void addProviders(boolean isServer, DataGenerator generator, PackOutput output, CompletableFuture<HolderLookup.Provider> provider, ExistingFileHelper helper) {
        generator.addProvider(isServer, new ModRegistryDataGenerator(output, provider));
        generator.addProvider(isServer, new ModDamageTypeTagGenerator(output, provider.thenApply(r -> append(r, BUILDER)), helper));
    }
