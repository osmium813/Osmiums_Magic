...
public class SynthesisRecipeBuilder {
    ...
    public SynthesisRecipeBuilder(RecipeCategory category, ItemLike result) {
        this.category = category;
        this.result = result.asItem();
    }

    public static SynthesisRecipeBuilder Synthesis(RecipeCategory category, ItemLike result) {
        return new SynthesisRecipeBuilder(category, result);
    }

    public SynthesisRecipeBuilder pattern(String line) {
        pattern.add(line);
        return this;
    }

    public SynthesisRecipeBuilder define(char symbol, ItemLike item) {
        key.put(symbol, Ingredient.of(item));
        return this;
    }

    public SynthesisRecipeBuilder crafttime(int time) {
        this.craftTime = time;
        return this;
    }

    public SynthesisRecipeBuilder needessence(int essence) {
        this.needEssence = essence;
        return this;
    }

    public SynthesisRecipeBuilder unlockedBy(String name, CriterionTriggerInstance criterion) {
        criteria.put(name, criterion);
        return this;
    }

    public void save(Consumer<FinishedRecipe> consumer) {
        save(consumer, Osmiumsmagic.MOD_ID + ":" + result.toString().toLowerCase() + "_synthesis");
    }

    public void save(Consumer<FinishedRecipe> consumer, String id) {
        consumer.accept(new SynthesisRecipeBuilder.Result(new ResourceLocation(id), result, key, pattern, craftTime, needEssence, criteria));
    }
    ...
    public record Result(ResourceLocation id, Item result, Map<Character, Ingredient> key, JsonArray pattern,
        ...
        public void serializeRecipeData(JsonObject json) {
            json.addProperty("type", Osmiumsmagic.MOD_ID + ":synthesis");
